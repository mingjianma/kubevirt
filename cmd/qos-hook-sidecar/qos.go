...
func onDefineDomain(vmiJSON []byte, domainXML []byte) ([]byte, error) {
	...
	for _, d := range domainSpec.Devices.Disks {
		disk := &d
		if disk.Type == "disk" {
			index := 0
			...
			if qos.TotalBytesSec != 0  {
				disk.IOTune.TotalBytesSec = qos.TotalBytesSec
			}else{
				disk.IOTune.ReadBytesSec = qos.ReadBytesSec
				disk.IOTune.WriteBytesSec = qos.WriteBytesSec
			}
			if qos.TotalIopsSec != 0 {
				disk.IOTune.TotalIopsSec = qos.TotalIopsSec
			}else{
				disk.IOTune.ReadIopsSec = qos.ReadBytesSec
				disk.IOTune.WriteIopsSec = qos.WriteIopsSec
			}
